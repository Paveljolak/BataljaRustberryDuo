...
// This is for storing the coordinates of our planets.
	public static String[] myCoordinatesX;
	public static String[] myCoordinatesY;

// This is for storing coordinates of enemy planets.
	public static String[] enemyCoordinatesX;
	public static String[] enemyCoordinatesY;
			...
			*/
			while (true) {
				/*
					...
					this class
				*/
				logToFile("something2");
				getGameState();
				
				logToFile("something3");
				logToFile("something4");
				...
				*/
				if (myColor.equals("blue")) {
					...
					myCoordinatesX = blueCoordinatesX;
					myCoordinatesY = blueCoordinatesY;
					...
					enemyCoordinatesX = joinArrays(greenCoordinatesX, yellowCoordinatesX, neutralCoordinatesX);
                    enemyCoordinatesY = joinArrays(greenCoordinatesY, yellowCoordinatesY, neutralCoordinatesY);
				} 
				...
				if (myColor.equals("cyan")) {
					...
					myCoordinatesX = cyanCoordinatesX;
					myCoordinatesY = cyanCoordinatesY;
					joinedPlanetNames = joinArrays(greenPlanets, yellowPlanets, neutralPlanets);
					joinedFleets = joinArrays(greenFleets, yellowFleets, neutralFleets);
					enemyCoordinatesX = joinArrays(greenCoordinatesX, yellowCoordinatesX, neutralCoordinatesX);
                    enemyCoordinatesY = joinArrays(greenCoordinatesY, yellowCoordinatesY, neutralCoordinatesY);
				...
				if (myColor.equals("green")) {
					...
					myCoordinatesX = greenCoordinatesX;
					myCoordinatesY = greenCoordinatesY;
					...
					enemyCoordinatesX = joinArrays(cyanCoordinatesX, blueCoordinatesX, neutralCoordinatesX);
                    enemyCoordinatesY = joinArrays(cyanCoordinatesY, blueCoordinatesY, neutralCoordinatesY);
				...
				if (myColor.equals("yellow")) {
					...
					myCoordinatesX = yellowCoordinatesX;
					myCoordinatesY = yellowCoordinatesY;
					joinedPlanetNames = joinArrays(cyanPlanets, bluePlanets, neutralPlanets);
					joinedFleets = joinArrays(cyanFleets, blueFleets, neutralFleets);
					enemyCoordinatesX = joinArrays(cyanCoordinatesX, blueCoordinatesX, neutralCoordinatesX);
                    enemyCoordinatesY = joinArrays(cyanCoordinatesY, blueCoordinatesY, neutralCoordinatesY);
			...
			if(joinedPlanetNames.length > 0 && myPlanets.length > 0) {
				...
				try {
					...
					for(int i = 0; i < myCoordinatesX.length; i++){
						int myX = Integer.parseInt(myCoordinatesX[i]);
						int myY = Integer.parseInt(myCoordinatesY[i]);
	
						
						int currMinDistance = Integer.MAX_VALUE;
						targetPlayer = joinedPlanetNames[i];
						for(int j = 0; j < enemyCoordinatesX.length; j++){
							int enemyX = Integer.parseInt(enemyCoordinatesX[j]);
							int enemyY = Integer.parseInt(enemyCoordinatesY[j]);
	
							
							int tempDistance = calculateDistance(myX, myY, enemyX, enemyY);
							if(tempDistance < currMinDistance){
								currMinDistance = tempDistance;
								targetPlayer = joinedPlanetNames[j];
							}
						}
	
						System.out.println("A " + myPlanets[i] + " " + targetPlayer);
					}
				...
				// 	int closestEnemyPlanet = Integer.parseInt(joinedFleets[1]);
               	// 	String closestEnemyPlanetName = joinedPlanetNames[1];

                //     for (int i = 1; i < myPlanets.length; i++) {
                //         for (int j = 1; j < joinedPlanetNames.length; j++) {
                           

                //             int temp = calculateDistance(
                //                     Integer.parseInt(myCoordinatesX[i]),
                //                     Integer.parseInt(myCoordinatesY[i]),
                //                     Integer.parseInt(enemyCoordinatesX[j]),
                //                     Integer.parseInt(enemyCoordinatesY[j]));
                //             if (temp < closestEnemyPlanet) {
                //                 closestEnemyPlanet = temp;
                //                 closestEnemyPlanetName = joinedPlanetNames[j];
                //             }
                //         }

                //         System.out.println("A " + myPlanets[i] + " " + closestEnemyPlanetName + " 30");
...
// ______________________________________________________________________________________________________________________________
	...
	public static void getGameState() throws NumberFormatException, IOException {
		...
		*/ 
		...
		neutralCoordinatesX = neutralCoordinateXList.toArray(new String[0]);
		...
		neutralCoordinatesY = neutralCoordinateYList.toArray(new String[0]);		
